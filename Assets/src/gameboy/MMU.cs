...
namespace src.gameboy
{
    public class MMU
    {
        private readonly IGamePack _gamePack;
        ...
        public bool TAC_ENABLED
        {
            get { return (IO[0x07] & 0x4) != 0; }
        } // Check if byte 2 is 1

        public byte TAC_FREQ
        {
            get { return (byte)(IO[0x07] & 0x3); }
        } // returns byte 0 and 1
        ...
        {
            if (addr >= 0x0000 && addr <= 0x3FFF) //0000-3FFF 16KB ROM Bank 00 (in cartridge, private at bank 00)
            {
                return _gamePack.ReadLoROM(addr);
            }

            if (addr >= 0x4000 && addr <= 0x7FFF) //4000-7FFF 16KB ROM Bank 01..NN(in cartridge, switchable bank number)
            {
                return _gamePack.ReadHiROM(addr);
            }
            ...
            {
                return _gamePack.ReadERAM(addr);
            }
            ...
            if (addr >= 0xE000 && addr <= 0xFDFF) //E000-FDFF Same as 0xC000-DDFF(ECHO)  
            {
                return WRAM0[addr & 0xFFF];
            }
            ...
            if (addr >= 0xFEA0 && addr <= 0xFEFF) // FEA0-FEFF Not Usable 0
            {
                return 0x00;
            }

            if (addr >= 0xFF00 && addr <= 0xFF7F) // FF00-FF7F IO Ports
            {
                return IO[addr & 0x7F];
            }

            if (addr >= 0xFF80 && addr <= 0xFFFF) // FF80-FFFE High RAM(HRAM)
            {
        ...
        public void writeByte(ushort addr, byte b)
        {
            switch (addr)
            {
                ...
                case ushort _
                    when addr <= 0x7FFF
                    : //0000-3FFF 16KB ROM Bank 00 (in cartridge, private at bank 00) 4000-7FFF 16KB ROM Bank 01..NN(in cartridge, switchable bank number)
                    _gamePack.WriteROM(addr, b);
                    break;
                ...
                case ushort _ when addr <= 0xBFFF: // A000-BFFF 8KB External RAM(in cartridge, switchable bank, if any)
                    _gamePack.WriteERAM(addr, b);
                    break;
                case ushort _ when addr <= 0xCFFF: // C000-CFFF 4KB Work RAM Bank 0(WRAM) <br/>
                    WRAM0[addr & 0xFFF] = b;
                    break;
                case ushort _
                    when addr <= 0xDFFF: // D000-DFFF 4KB Work RAM Bank 1(WRAM)(switchable bank 1-7 in CGB Mode)
                    WRAM1[addr & 0xFFF] = b;
                    break;
                ...
                case ushort _ when addr <= 0xFF7F: // FF00-FF7F IO Ports
                    switch (addr)
                    {
                        case 0xFF0F:
                            b |= 0xE0;
                            break; // IF returns 1 on first 3 unused bits
