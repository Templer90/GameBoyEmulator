...
{
     class MBC1 : IGamePack {

        private byte[] _rom;
        private readonly byte[] _eram = new byte [0x8000]; //MBC1 MAX ERAM on 4 banks
        ...
        public byte ReadERAM(ushort addr) {
            if (ERAM_ENABLED){
                return _eram[(ERAM_OFFSET * RAM_BANK) + (addr & 0x1FFF)];
            } else {
                return 0xFF;
            }
        }
        ...
        public void WriteERAM(ushort addr, byte value) {
            if (ERAM_ENABLED) {
               _eram[(ERAM_OFFSET * RAM_BANK) + addr & 0x1FFF] = value;
            }
        }
